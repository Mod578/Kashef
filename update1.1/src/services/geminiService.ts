...
export class GeminiService {
    ...
    /**
     * Starts a new chat session with a given system instruction.
     * @param systemInstruction The system-level instruction for the chat model.
     * @returns A `Chat` instance from the Gemini SDK.
     */
    startChatSession(systemInstruction: string): Chat {
     ...
     * @param componentType The type of the component (e.g., 'GPU', 'CPU').
     * @returns A base64-encoded data URL for the generated image, or null if generation fails.
     */
    async generateComponentImage(componentName: string, componentType: string): Promise<string | null> {
        try {
            const response = await this.ai.models.generateImages({
                model: 'imagen-4.0-generate-001',
                prompt: `**Task:** Generate an ultra-realistic, 4K studio product photograph of a specific computer hardware component.

                **Component to Photograph:**
                - **Name:** "${componentName}"
                - **Type:** Computer ${componentType}
                
                **Visual Style & Composition:**
                - **Composition:** The component must be the *only* object, perfectly centered. It is crucial that the component occupies approximately 75% of the image frame, creating a generous margin of the background on all sides. The component must not touch the top, bottom, left, or right edges of the photograph.
                - **Background:** Solid, neutral, non-distracting dark gray background.
                - **Lighting:** Professional, soft, multi-point studio lighting to highlight details without creating harsh glares or deep shadows.
                - **Quality:** Photorealistic, sharp, detailed, and high-resolution. It should look like a professional product shot for a tech website.
                
                **CRITICAL INSTRUCTIONS - DO NOT INCLUDE:**
                - **No other objects:** Absolutely no people, hands, tools, cables, other components, or unrelated items.
                - **No environments:** Do not place the component on a desk or in any other environment.
                - **No text/graphics:** No watermarks, text overlays, or logos (except those inherent to the component's design).
                - **No artistic styles:** The output must be a photograph, not a drawing, 3D render, or illustration.`,
                config: {
                    numberOfImages: 1,
...
4.  **الإخراج:** قم بإرجاع مصفوفة فارغة إذا لم يتم التعرف على أي مكونات مؤكدة.`,
        ...
        try {
            ...
            if (result && result.components && Array.isArray(result.components)) {
                // Add a unique, deterministic ID to each component using a Unicode-safe base64 encoding of its name.
                return result.components.map((c: Omit<DetectedComponent, 'id'>) => ({
                    ...c,
                    id: `comp-${toBase64(c.name)}`,
                }));
            }
            return [];
