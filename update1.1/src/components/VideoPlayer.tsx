    ...
    return (
        <div className="relative w-full h-full flex flex-col bg-black">
            ...
            <div className="absolute bottom-[calc(1rem+env(safe-area-inset-bottom))] sm:bottom-4 left-1/2 -translate-x-1/2 z-30 flex items-center gap-4 transition-all duration-300">
                {(analysisMode === 'camera' || analysisMode === 'image' || analysisMode === 'video') && (
                    <button
                        onClick={resetAnalysis}
                        disabled={isProcessingFrame}
                        className="w-14 h-14 bg-brand-surface text-brand-text-secondary rounded-full flex items-center justify-center border-4 border-brand-surface disabled:opacity-50 hover:bg-brand-surface-alt focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-brand-bg focus-visible:ring-brand-primary transition-all active:scale-95 shadow-lg"
                        aria-label={analysisMode === 'camera' ? 'إغلاق الكاميرا' : 'إلغاء واختيار ملف آخر'}
                    >
