...
/**
 * Renders an item representing a hardware slot in the system (e.g., CPU, RAM).
 * It displays the slot's label, icon, and information about any detected component.
 * It can be in various states: empty, detected, selected, or loading.
 *
 * @param {SystemSlotItemProps} props - The component props.
 * @returns {React.ReactElement} The rendered system slot item.
 */
const SystemSlotItem: React.FC<SystemSlotItemProps> = ({ slot, isSelected, isLoading, onSelect }) => {
    ...
    const baseClasses = 'relative w-full text-right p-2 pr-3 flex items-center gap-3 border rounded-xl transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-brand-bg focus-visible:ring-brand-primary';
    
    const stateClasses = isDetected
        ? isSelected
            ? 'bg-brand-primary/10 cursor-pointer border-transparent'
            : 'bg-brand-surface border-brand-border hover:border-brand-primary/50 hover:shadow-md cursor-pointer'
        : 'bg-brand-surface border-transparent opacity-60 cursor-not-allowed';
    ...
    return (
        ...
        >
            ...
            <div className={`flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg border transition-colors ${isSelected ? 'bg-brand-primary/20 border-brand-primary/30' : isDetected ? 'bg-brand-surface-alt border-brand-border' : 'bg-brand-surface border-brand-border'}`}>
