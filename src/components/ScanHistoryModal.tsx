...
import { FaTimes, FaTrash, FaHistory, FaDownload } from 'react-icons/fa';
...
/**
 * A modal component that displays a history of saved component scans.
 * It allows users to view, load, or delete previous scans.
 *
 * @param {ScanHistoryModalProps} props - The component props.
...
const ScanHistoryModal: React.FC<ScanHistoryModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleDeleteScan = (scanId: string) => {
        if (window.confirm("هل أنت متأكد من رغبتك في حذف هذا الفحص؟ لا يمكن التراجع عن هذا الإجراء.")) {
                                ...
                                <li key={scan.id} className="group bg-brand-surface border border-brand-border rounded-xl p-3 flex items-center justify-between gap-3 transition-shadow hover:shadow-lg animate-fade-in-up">
                                    ...
                                    <div className="flex-shrink-0 flex items-center gap-2">
                                        ...
                                        <button
                                            ...
                                            className="w-9 h-9 flex items-center justify-center bg-brand-surface-alt text-brand-text-secondary rounded-lg hover:bg-brand-danger/20 hover:text-brand-danger transition-colors"
                                            title="حذف الفحص"
                                        >
