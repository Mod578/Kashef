// FIX: Implemented the ChatAssistant component to provide a functional chat interface, handle chat sessions with Gemini, and resolve module errors.
...
const ChatAssistant: React.FC<ChatAssistantProps> = ({ 
    ...
    const { selectedComponent, isDemoMode } = state;
    ...
    const isFormDisabled = isDemoMode || !chatSession || isLoading;
    ...
    const handleReset = () => {
        if (window.confirm("هل أنت متأكد من رغبتك في إعادة تعيين المحادثة؟")) {
            onResetChat();
        }
    }
    ...
    const isOnline = !isDemoMode && !!chatSession;
    ...
    return (
        <Dashboard
            title="المساعد الذكي"
            subtitle={isDemoMode ? 'عرض سجل المحادثة' : 'اسأل عن أي شيء متعلق بمكونات الكمبيوتر'}
            icon={AssistantIcon}
            footer={
            ...
            <div className="h-full flex flex-col">
                ...
                {chatHistory.length === 0 ? (
                    ...
                    <>
                        ...
                        {!isDemoMode && (
                            <button onClick={handleReset} className="mt-4 mx-auto flex items-center gap-2 text-xs text-brand-text-tertiary hover:text-brand-text-secondary transition-colors">
                                <GrPowerReset />
                                <span>إعادة تعيين المحادثة</span>
                            </button>
                        )}
                    </>
                )}
